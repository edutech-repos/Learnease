...
import { igniteLesson, igniteLessonMock, saveStudyMaterial, DEFAULT_DIAGRAM_PATH } from '../lib/api';
...
export function DashboardPremium({ userName, userId = 'USR_TEST_001', onNavigate, onShowProfile, onLogout, onShowComparison }: DashboardPremiumProps) {
  ...
  // Handle save content to Supabase
  const handleSaveContent = async () => {
    ...
    try {
      const response = await saveStudyMaterial({
        userId: userId,
        title: originalContent.substring(0, 50) + '...',
      ...
      setApiError('Failed to save content. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleShowDiagram = async () => {
    if (!isSaved) {
      ...
      setApiError('Please save your content first to generate a diagram!');
      return;
    }
            ...
            >
              ...
              {generatedContent && (
                <div className="mt-8 flex flex-col md:flex-row gap-4">
                  {/* Save Button - Glows when unsaved */}
                  <motion.button
                    onClick={handleSaveContent}
                    disabled={isSaving || isSaved}
                    className={`flex-1 py-3 rounded-xl flex items-center justify-center gap-2 relative overflow-hidden ${isSaved
                    ...
                    transition={!isSaved && !isSaving ? { duration: 1.5, repeat: Infinity } : undefined}
                  >
                    {isSaving ? (
                      <>
                        <Loader2 className="w-5 h-5 animate-spin" />
                      ...
                      <>
                        <Save className="w-5 h-5" />
                        <span className="font-medium">Save Lesson</span>
                      </>
                    )}
                  </motion.button>
                  ...
                  <motion.button
                    ...
                    className={`flex-1 py-3 bg-[#1E1B4B] border-2 rounded-xl flex items-center justify-center gap-2 relative overflow-hidden group ${isSaved ? 'border-[#06B6D4]/50 text-[#06B6D4]' : 'border-gray-600 text-gray-500'
                      }`}
                    whileHover={isSaved ? {
                      scale: 1.02,
                      borderColor: '#06B6D4',
                  ...
                  >
                    ...
                    {!isSaved && <span className="text-xs ml-1">(Save first)</span>}
                  </motion.button>
                ...
                {showDiagram && (
                  ...
                  >
                    ...
                    <div className="flex justify-center bg-white p-4">
                      <img
                        src={savedImagePath ? `https://mpesgdkzwfthszeopavy.supabase.co/storage/v1/object/public/user-diagrams/${savedImagePath}` : DEFAULT_DIAGRAM_PATH}
                        alt="AI Generated Diagram"
                        className="max-w-full h-auto max-h-[400px] object-contain"
                        onError={(e) => {
                          // Fallback to default if image fails to load
                          (e.target as HTMLImageElement).src = DEFAULT_DIAGRAM_PATH;
                        }}
                      />
                    </div>
                  </motion.div>
